 ...
 * dp[i][w] : stores the max value for the first i items and a knapsack with weight capacity w
 * recursive call : if dp[i][w] is already computed, return its value
 * otherwise compute with current item is included or excluded and store result in the table
 * 
 * Base case : i == 0 or w == 0 then result is 0
 * 
 * final result : result is stored in dp[n][W] representing value obtainable with n items and 
 * a capacity of W
 * 
 * TC : dp has n X W dimensions
 * each subproblem is solved once and stored
 * total number of states is O(nXW)
 * each state is O(1), hence TC O(n x W)
 * 
 * SC : memoization table required O(n x W space)
 * recursion depth O n ,  hence SC O(n x W)
 * 
 */
public class knapsackII {
    ...
    public static int findMaxKnapsackProfitHelper(int capacity, int[]  weights, int[]  values, int n, int [][] dp) {
        ...
        // if we already solved subproblem, fetch the result from memory
        if (dp[n][capacity] != -1) {
